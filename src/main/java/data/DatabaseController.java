...
public class DatabaseController
{
	...
	private static String titlesTable = "Titles";
	private static String provTitlesTable = "ProvTitles";
	// jdbc Connection
	 ...
	 */
	public static void insertProvider(int provID, String lname, String fname, String title)
	{
		try
		{
			stmt = connection.createStatement();
			stmt.execute("insert into " + providerTable + " values (" + provID + ", '" + lname + "', '" +
					fname + "', '" + title + "')");
			stmt.close();
		} catch (SQLException sqlExcept)
		{
	...
	//initializes the Titles DB table
	public static void initializeTitlesTable(){
		try
		{
			stmt = connection.createStatement();
			//TODO: delete following line after hard coding is done
			...
			if (!sqlExcept.getSQLState().equals("X0Y32"))
			{
				...
				System.out.println("Titles table already exists");
			}
		}
	}

	//inserts into the Services Table
	public static void insertTitles(String acronym, String titleName) {
		try
		{
			stmt = connection.createStatement();
			stmt.execute("insert into " + titlesTable + " values ('" + acronym + "', '" + titleName + "')");
			stmt.close();
		}
		catch (SQLException sqlExcept)
		{
