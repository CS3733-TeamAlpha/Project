...
public class TextualDirections
{
	...
	public static ArrayList<String> getDirections(ArrayList<Node> path, ArrayList<Node> totalPath)
	{
		...
		for (int i = length - 2; i >= 2 ; i--)
		{
			...
			if (path.get(i).getType() == 2 && path.get(i - 1).getType() == 2)
			{
				toReturn.add("Take the elevator to floor " + path.get(i-1).getFloor());
				i--;
			}
			else if (path.get(i).getType() == 20 && path.get(i - 1).getType() == NodeTypes.STAIRWAY.val())
